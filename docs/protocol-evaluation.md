...
**已将 `HoneyBBQ/teamspeak-go` 固定快照用于 M1 只读实验适配器；暂缓将其作为完整语音 GUI 或长期运行客户端的已验证基础。** 采用边界与补丁见 ADR-0005，最新执行结果见验证记录；下面保留初次源码评估及其证据边界。它实现普通 TS3 客户端 UDP 连接路径，提供文字、频道查询、身份和原始 Opus 发送接口。然而，上层客户端直接丢弃收到的语音包，不能据此承诺双向语音。
