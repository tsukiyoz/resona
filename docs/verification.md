...
2026-09-07，用户体验当前桌面版后确认“验收，暂无问题”，授权提交本轮频道导航、成员显示、提示音、记住密码与服务器进入流程。以上自动化及真实集成结果作为本次提交的验证依据；用户反馈不代表下列尚未实现功能或跨平台兼容矩阵已通过。
...
- 真实文字通信、密码频道切换、语音收发、长连接、真实被踢/断网场景及自动重连。专用频道的实际应用服务切换已验证，原生 GUI 点击流程仍待验证；不代表完整频道权限矩阵。
